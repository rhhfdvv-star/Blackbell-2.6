...
async function startServer() {
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      const userName = isAdminUser ? "Sachin" : (db.users.find(u => u.email === userEmail)?.username || "User");
...
5. CRITICAL: At the very end of your response, you MUST ALWAYS generate exactly 3 short, relevant, highly clickable follow-up questions that the user might ask next. They must be matching your flirty, sweet and charming companion style. Place them EXACTLY in this format at the very bottom:
[QUESTIONS]
- Question 1
- Question 2
- Question 3
...
3. If there is absolutely nothing new or useful to log, output EXACTLY the word "NONE". Do not reply with extra text or explanations.`;
  ...
  wss.on("connection", async (clientWs, req) => {
    try {
      ...
      const liveUserName = isAdminLive ? "Sachin" : (db.users.find(u => u.email === userEmail)?.username || "User");
